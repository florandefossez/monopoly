...
class Entrie:
    ...
    def update(self, font):
        ...
        self.amount_surface = font.render(f"{self.amount} $", True, pygame.Color(31,165,76) if self.amount>=0 else pygame.Color(227,1,15), pygame.Color(218, 233, 212))
    ...
    def draw(self, rect):
        ...
        self.game.screen.blit(
            self.amount_surface,
            rect.move(-self.amount_surface.get_width(),0).topright
        )
...
class Bill:
    ...
    def update(self):
        ...
        self.rect = pygame.Rect((self.game.width - self.game.height)/4,0,(self.game.width - self.game.height)/2,int(self.game.height//35))
