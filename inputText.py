    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.rect.collidepoint(event.pos):
                ...
                self.is_active = False
            self.color = InputText.color_active if self.is_active else InputText.color_inactive
        if event.type == pygame.KEYDOWN:
            if self.is_active:
                ...
                elif (self.force_numeric and event.unicode.isdecimal()) or not self.force_numeric:
